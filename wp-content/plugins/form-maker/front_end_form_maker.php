    ...
    switch ($type) {
      ...
      case "type_checkbox":
        ...
        case "type_paypal_price": {		
          ...
          $quantity = ((isset($_POST[$i . "_element_quantity" . $id]) && ($_POST[$i . "_element_quantity" . $id] >= 1)) ? $_POST[$i . "_element_quantity" . $id] : 1);
					$total += (float)($_POST[$i . "_element" . $id]) * (float)($quantity);
					array_push($paypal['item_name'], $label_label[$key] . ' ' . $_POST[$i."_element_label".$id]);
					array_push($paypal['quantity'], $quantity);
					array_push($paypal['amount'], $_POST[$i."_element".$id]);
					...
					for($k=0; $k<50; $k++) {
						...
						if(isset($temp_val)) {			
							array_push($paypal_option['on'], $_POST[$i."_element_property_label".$id.$k]);
							array_push($paypal_option['os'], $_POST[$i."_element_property_value".$id.$k]);
          ...
          if (isset($_POST[$i."_element_label".$id]) && ($_POST[$i."_element_label".$id] != '')) {
            $value = $_POST[$i."_element_label".$id].' - '.$_POST[$i."_element".$id].$form_currency;
            $paypal['shipping'] = $_POST[$i."_element".$id];
          }
					break;
				}
					...
					if ($start != -1) {
						for ($j = $start; $j < 100; $j++) {
							...
							if ($j == $other_element_id) {
								...
								$value=$value.$_POST[$i."_element".$id.$j."_label"].' - '.($_POST[$i."_element".$id.$j]=='' ? '0' : $_POST[$i."_element".$id.$j]).$form_currency.'***br***';
								$quantity = ((isset($_POST[$i . "_element_quantity" . $id]) && ($_POST[$i . "_element_quantity" . $id] >= 1)) ? $_POST[$i . "_element_quantity" . $id] : 1);
                $total += (float)($_POST[$i."_element".$id.$j]) * (float)($quantity);
								array_push($paypal['item_name'], $label_label[$key].' '.$_POST[$i."_element".$id.$j."_label"]);
								array_push($paypal['quantity'], $quantity);
            ...
            $quantity = ((isset($_POST[$i . "_element_quantity" . $id]) && ($_POST[$i . "_element_quantity" . $id] >= 1)) ? $_POST[$i . "_element_quantity" . $id] : 1);
						if (isset($element_quantity_label)) {
							$value .= $_POST[$i."_element_quantity_label".$id].': '.$quantity.'***br***';
            }
						for ($k = 0; $k < 50; $k++) {
							$temp_val = $_POST[$i."_element_property_value".$id.$k];
							if (isset($temp_val)) {			
								$value .= $_POST[$i."_element_property_label".$id.$k].': '.$_POST[$i."_element_property_value".$id.$k].'***br***';
							}
						}
					}
					break;
...
function form_maker_front_end($id) {
  ...
  if ($row->form_front) {
    ...
    $form_maker_front_end .= 'RangeFieldMsg	=\'' . addslashes(__('The', 'form_maker')) . ' `FIELDNAME` ' . addslashes(__('value must be between', 'form_maker')) . ' `FROM` - `TO`\';';
    $form_maker_front_end .= 'FormCurrency = "' . $form_currency . '";';
    $form_maker_front_end .= 'FormPaypalTax = ' . $form_paypal_tax . ';';
...
{';
	...
	{
		if(document.getElementById('".$label_id[$key]."_element_dollars".$id."').title!='".addslashes($_POST[$label_id[$key]."_element_dollars".$id])."')
		{	document.getElementById('".$label_id[$key]."_element_dollars".$id."').value='".addslashes($_POST[$label_id[$key]."_element_dollars".$id])."';
			document.getElementById('".$label_id[$key]."_element_dollars".$id."').className='input_active';
		}
		
		if(document.getElementById('".$label_id[$key]."_element_cents".$id."').title!='".addslashes($_POST[$label_id[$key]."_element_cents".$id])."')
		{	document.getElementById('".$label_id[$key]."_element_cents".$id."').value='".addslashes($_POST[$label_id[$key]."_element_cents".$id])."';
	...
	if(document.getElementById('".$label_id[$key]."_element_quantity".$id."'))
		...
		for($j=0; $j<100; $j++)
								{
										$element=$_POST[$label_id[$key]."_property".$id.$j];
										if(isset($element))
												{
												$form_maker_front_end .= 
	"document.getElementById('".$label_id[$key]."_property".$id.$j."').value='".addslashes($_POST[$label_id[$key]."_property".$id.$j])."';
	";
												}
								}
		$form_maker_front_end .= "
